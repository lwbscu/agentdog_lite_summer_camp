...
本项目实现 AgentDoG-Lite 题目 1 的高标准方案：最终主线从基础模型 `Qwen/Qwen3.5-0.8B` 出发，经过 LoRA/SFT 与 Full-SFT 训练，构建面向 Agent 轨迹级安全判断的轻量模型。原始 `Qwen/Qwen3.5-0.8B` 既作为比赛对照 baseline，也是本项目最终主线训练起点；官方 `AI45Research/AgentDoG1.5-Qwen3.5-0.8B` 仅作为 reference 对照模型，用于比较官方 AgentDoG1.5 原始权重表现，不作为本项目最终主线初始化权重。

当前实验主线同时包含一套全参 Full-SFT + batch eval 闭环：使用本地 H800 环境对 `Qwen/Qwen3.5-0.8B` 做完整权重 SFT，每约 30 分钟保存 checkpoint 并立即在 summer camp test set 上 batch eval；只保留 best/latest 权重以节省磁盘。当前推荐本地权重入口为：

```text
outputs/qwen35_full_sft_llamafactory_h800/best_checkpoint

如果使用 LoRA step48 merge 后继续 Full-SFT，则当前最佳打包模型为：

李文博_best_lora_step48_plus_fullsft_step41/model

该目录是完整模型 checkpoint，可直接通过 AutoModelForCausalLM.from_pretrained() 加载。
...
火山 8xL40 服务器使用 PyTorch cu121，不使用 cu128；完整部署提示词见：

docs/volcano_l40_setup_prompt.md
官方仓库
...
method 名称	Hugging Face 模型	本地路径	作用
qwen35_08b_baseline	Qwen/Qwen3.5-0.8B	models/Qwen3.5-0.8B	基础模型，也是本项目最终主线训练起点
agentdog15_08b_reference	AI45Research/AgentDoG1.5-Qwen3.5-0.8B	models/AgentDoG1.5-Qwen3.5-0.8B	官方 AgentDoG1.5 reference 对照
teacher	AI45Research/AgentDoG1.5-FG-Qwen3.5-0.8B	models/AgentDoG1.5-FG-Qwen3.5-0.8B	细粒度 teacher/reference，用于辅助分析
数据下载
...
train_mixed 优先按 70% binary、20% diagnostic、10% hard boundary 构造；如果 hard boundary 数量不足以支撑稳定训练规模，则按规则回退到 75% binary、25% diagnostic。

dev split 来自训练集内部，按 task/source/judgment 分层，绝不使用 summer camp test set 做 dev、few-shot 或 prompt tuning。

Continued LoRA 训练

本项目最终主线 LoRA 从基础模型 Qwen/Qwen3.5-0.8B 开始训练。

H800 主线配置：

python scripts/train_lora.py --config configs/train_qwen35_fallback_lora_h800.yaml

官方 AgentDoG1.5 reference 的继续训练配置可以保留为对照实验：
...
但在最终说明中需要明确：这些是 reference/ablation，不是最终主线。

训练脚本会校验 H800 配置的 effective batch size 必须为 128。默认设置为：
...
训练只对 assistant JSON target 计算 loss，system/user/trajectory prompt token 的 label 均为 -100。每次训练都会写 TensorBoard 日志到：
...
python scripts/export_final_adapter.py \
  --run-dir outputs/qwen35_lora_hardcase_from_best_conservative \
  --output-dir outputs/final_qwen35_lora_adapter

如果使用历史 LoRA 输出，最佳 LoRA checkpoint 为：

outputs/qwen35_lora_hardcase_from_best_conservative/checkpoints/step_48_20260705_100017

完整 LoRA adapter 目录应至少包含：

adapter_config.json
adapter_model.safetensors
lora_checkpoint_metadata.json

其中：

adapter_config.json 是 LoRA 结构配置；
adapter_model.safetensors 是 LoRA 真正训练出来的权重；
lora_checkpoint_metadata.json 是 checkpoint 元信息。
Full-SFT 训练
...
Full-SFT 主线基于 Qwen/Qwen3.5-0.8B 相关训练产物继续展开，最终推荐权重入口为：

outputs/qwen35_full_sft_llamafactory_h800/best_checkpoint

如果使用 LoRA step48 merge 后继续 Full-SFT，则最终打包模型为：

李文博_best_lora_step48_plus_fullsft_step41/model

该目录是完整模型 checkpoint，可直接加载。
...
表示未经训练的基础模型 Qwen/Qwen3.5-0.8B，用于对照训练前表现。

agentdog15_08b_reference

表示官方 AI45Research/AgentDoG1.5-Qwen3.5-0.8B 原始权重，用作 reference，不叫 baseline，也不是本项目最终主线起点。

评测实现为 batch generate。默认：

eval_batch_size = 64

遇到 OOM 或 canUse32BitIndexMath 等可恢复错误时自动降到：

32
16
8

并在 summary.json 中记录实际 batch size。
...
每次评测都会写 TensorBoard 日志到：

logs/only_eval/李文博_<method>_<YYYYmmdd_HHMMSS>/

并在对应 summary.json 中记录 tensorboard_log_dir。

Final Eval

训练并导出最终模型后：

python scripts/evaluate.py --method qwen35_full_sft_llamafactory_best
...
outputs/qwen35_full_sft_llamafactory_h800/
...
李文博_best_lora_step48_plus_fullsft_step41
...
combined overall accuracy: 0.7870370370370371
ATBench accuracy / macro F1: 0.620000 / 0.5893174504058792
R-Judge accuracy / macro F1: 0.875886524822695 / 0.875886524822695

该模型是完整 Full-SFT model，可直接通过：

from transformers import AutoTokenizer, AutoModelForCausalLM
import torch

model_path = "李文博_best_lora_step48_plus_fullsft_step41/model"
...
如果上传完整 Full-SFT 模型，建议仓库名：

hhggfdd/agentdog-lite-safir-full-sft
...
cd /path/to/李文博_best_lora_step48_plus_fullsft_step41

hf auth login

hf repo create hhggfdd/agentdog-lite-safir-full-sft --type model --private
...
https://huggingface.co/hhggfdd/agentdog-lite-safir-full-sft

如果上传 LoRA adapter，则仓库名可以使用：

hhggfdd/agentdog-lite-safir-lora
...
无法解析时默认 unsafe，并计入 invalid output。
...
如果使用 summer camp test set 的 eval 结果选择 checkpoint、调 prompt 或迭代数据集，该 eval set 已参与开发决策，不能再作为完全无偏最终测试集。
